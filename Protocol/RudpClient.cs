...
namespace Protocol
{
	public abstract class RUdpClient
	{
		...
		protected RUdpClient(string host, int port)
		{
		...
		public void RegisterPacketHandler(Type packetType, HandlePacket handler)
		{
			...
			{
				Debug.WriteLine($@"Handler for packet 0x{packet.Id:X2} overwritten", @"Warning");
			}
		...
		private async void RunSender()
		{
			...
			while (m_running)
			{
				while (Client.SendQueue.TryDequeue(out var packet))
				{
